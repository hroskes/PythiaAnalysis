...
private:
  ...
  void getPairMass(const reco::Candidate* lp, const reco::Candidate* lm,  float& mass, int& ID);
  void buildMELA();
...
//  edm::EDGetTokenT<edm::View<reco::GenJet> > jetToken;
  edm::EDGetTokenT<reco::GenJetCollection > jetToken;

  //edm::EDGetTokenT<edm::View<reco::Candidate> > softLeptonToken;
  edm::EDGetTokenT<reco::CandidateCollection > softLeptonToken;
  ...
  ZRolesByMass(iConfig.getParameter<bool>("ZRolesByMass"))
{
  ...
  jetToken = consumes<reco::GenJetCollection> (edm::InputTag("cleanJets"));
  softLeptonToken = consumes<reco::CandidateCollection >(edm::InputTag("softLeptons"));
  ZCandToken = consumes<edm::View<reco::CompositeCandidate> >(edm::InputTag("ZCand"));


//  jetToken = consumes<edm::View<reco::GenJet> >(iConfig.getParameter<edm::InputTag>("genjet"));
//  softLeptonToken=(consumes<edm::View<reco::Candidate> >( iConfig.getParameter<edm::InputTag>("pruned")));
...
void ZZCandidateFiller::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  // Get jets
  Handle<reco::GenJetCollection> CleanJets;
//  Handle<edm::View<reco::GenJet> > CleanJets;
  ...
  for( View<CompositeCandidate>::const_iterator cand = LLLLCands->begin(); cand != LLLLCands->end(); ++ cand ) {
    ...
    float mZa, mZb;
    int ZaID, ZbID;
    getPairMass(Z1Lp,Z2Lm,mZa,ZaID);
    getPairMass(Z1Lm,Z2Lp,mZb,ZbID);
    ...
    getPairMass(Z1Lp,Z2Lp,mZalpha,ZalphaID);
    getPairMass(Z1Lm,Z2Lm,mZbeta,ZbetaID);
...
//    if((bool)userdatahelpers::getUserFloat(&*myZCand,"GoodIsoLeptons")){
 ...
 //   }
    ...
    for (int jecnum = 0; jecnum < 1; jecnum++){
...
//        float jec_unc = jet->userFloat("jec_unc");
	...
	//std::cout<< jet->pt()<<"\t"<<jet->phi()<<std::endl;
      }
      if (jecnum==0 && cleanedJetsPt30Jec.size()>1){
        const reco::GenJet& jet1 = *(cleanedJetsPt30Jec.at(0));
        const reco::GenJet& jet2 = *(cleanedJetsPt30Jec.at(1));
//	std::cout<< jet1.pt()<<"\t"<<jet1.phi()<<std::endl;
//	std::cout<< jet2.pt()<<"\t"<<jet2.phi()<<std::endl;
...
void
ZZCandidateFiller::getPairMass(const reco::Candidate* lp, const reco::Candidate* lm, float& mass, int& ID){
